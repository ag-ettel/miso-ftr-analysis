...
builder = SparkSession.builder \
    .appName("MISO LMP RT Exante Production") \
...
## main function to fetch DA ex-ante LMP data
def fetch_lmp_da_exante_for_date(target_date):
    """
    Fetch DA ex-ante LMP data from MISO API for a specific date and list of target nodes
    ...
    for node in TARGET_NODES:
        ...
        while page <= max_pages:
            url = f"{API_BASE}/day-ahead/{date_str}/lmp-exante?node={node}&pageNumber={page}"
...
def process_date_range(start_date, end_date):
    ...
    while current_date <= end_date:
        ...
        date_records = fetch_lmp_da_exante_for_date(current_date)
...
try:
    ...
    start_date = today - timedelta(days=11) # add_years(today, -3)  # 3 years ago
