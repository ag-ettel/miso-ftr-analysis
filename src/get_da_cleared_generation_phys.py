...
builder = SparkSession.builder \
    .appName("MISO DA Cleared Generation Physical") \
...
OUTPUT_DIR = "data/miso_da_cleared_generation_phys"
...
## main function to fetch DA cleared generation physical data
def fetch_da_cleared_generation_phys_for_date(target_date):
    """
    Fetch DA cleared generation physical data from MISO API for a specific date and target region
    ...
    for region in TARGET_REGIONS:
        print(f" Fetching DA cleared generation physical for {date_str} in region {region}...")

        page = 1
        max_pages = 10

        while page <= max_pages:
            url = f"{API_BASE}/day-ahead/{date_str}/generation/cleared/physical?pageNumber={page}&region={region}"
            ...
            for item in data:
                ...
                record = {
                    ...
                    'demand_type': 'da_cleared_generation_phys'
                }
...
def process_date_range(start_date, end_date):
    """
    Process DA cleared generation physical for date range and save to Delta table
    ...
    while current_date <= end_date:
        ...
        date_records = fetch_da_cleared_generation_phys_for_date(current_date)
    ...
    if all_records:
        ...
        quality_df = processed_df.filter(
            (col("supply").isNotNull()) & (col("supply") >= 0)
        )
        ...
        print(f"Saved {record_count} DA cleared generation physical records to {OUTPUT_DIR}")
        return record_count
    else:
...
try:
  ...
  #  print(f"Starting DA cleared generation physical data processing from {start_date} to {end_date}...")

    
   # Each winter season runs December-February
    # 2018 -> Winter 2018-2019 (captures Jan 2019 Polar Vortex)
    # 2020 -> Winter 2020-2021 (captures Feb 2021 Uri)
    # 2022 -> Winter 2022-2023 (captures Dec 2022 Elliott)
