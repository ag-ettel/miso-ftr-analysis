...
OUTPUT_DIR = "data/miso_da_fuel_mix"
...
def fetch_da_fuel_mix_for_date(target_date):
    """
    Fetch DA fuel mix data from MISO API for a specific date and target region
    ...
    for region in TARGET_REGIONS:
        print(f" Fetching DA fuel mix for {date_str} in region {region}...")

        page = 1
        max_pages = 10
        while page <= max_pages:
            url = f"{API_BASE}/day-ahead/{date_str}/generation/fuel-type?pageNumber={page}&region={region}"
...
def process_date_range(start_date, end_date):
    """
    Process DA fuel mix for date range and save to Delta table
    ...
    while current_date <= end_date:
        ...
        date_records = fetch_da_fuel_mix_for_date(current_date)
    ...
    if all_records:
        ...
        print(f"Saved {record_count} DA fuel mix records to {OUTPUT_DIR}")
        return record_count
    else:
