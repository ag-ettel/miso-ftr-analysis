...
builder = SparkSession.builder \
    .appName("MISO DA Cleared Generation Virtual") \
...
OUTPUT_DIR = "data/miso_da_cleared_generation_virtual"
...
## region of interest
TARGET_REGION = "NO_REGION" # virtual generation is region-less

## main function to fetch DA cleared generation virtual data
def fetch_da_cleared_generation_virtual_for_date(target_date):
    """
    Fetch DA cleared generation virtual data from MISO API for a specific date and target region
    ...
    print(f" Fetching DA cleared generation virtual for {date_str} in region {TARGET_REGION}...")

    page = 1
    max_pages = 10

    while page <= max_pages:
        url = f"{API_BASE}/day-ahead/{date_str}/generation/cleared/virtual?pageNumber={page}&region={TARGET_REGION}"
        ...
        for item in data:
            ...
            record = {
                ...
                'demand_type': 'da_cleared_generation_virtual'
            }
...
def process_date_range(start_date, end_date):
    """
    Process DA cleared generation virtual for date range and save to Delta table
    ...
    while current_date <= end_date:
        ...
        date_records = fetch_da_cleared_generation_virtual_for_date(current_date)
    ...
    if all_records:
        ...
        print(f"Saved {record_count} DA cleared generation virtual records to {OUTPUT_DIR}")
        return record_count
    else:
...
#    print(f"Starting DA cleared generation virtual data processing from {start_date} to {end_date}...")
